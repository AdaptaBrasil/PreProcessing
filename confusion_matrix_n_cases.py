...
def main(args):
    ...
    all_indicator_shapefile_paths = glob.glob(
        indicator_files_mask, recursive=True)

    indicators_spreadsheet_df = pd.read_excel(indicators_spreadsheet)
    if debug:
        print("Indicators spreadsheet head:  ",
              indicators_spreadsheet_df.head())

    # Remove the row at index 0 from the dataframe
    indicators_spreadsheet_df = indicators_spreadsheet_df.drop(
        indicators_spreadsheet_df.index[0])

    # Reorder the index
    indicators_spreadsheet_df.index = range(len(indicators_spreadsheet_df))

    # Rename the first column to 'objectid'
    indicators_spreadsheet_df = indicators_spreadsheet_df.rename(
        columns={indicators_spreadsheet_df.columns[0]: 'objectid'})
    ...
    indicators_spreadsheet_df = indicators_spreadsheet_df.drop(
        columns=[indicators_spreadsheet_df.columns[0]])
    ...
    for i_column, column_name in enumerate(indicators_spreadsheet_df.columns):
        ...
        indicator_shp_path = find_path_containing_string(
            indicator_name, all_indicator_shapefile_paths)
        ...
        indicator_df = pd.concat(
            [objectid_column_df, indicators_spreadsheet_df.iloc[:, [i_column]]], axis=1)
        ...
        intersection = gpd.sjoin(
            indicator, mesh_merge, how='inner', predicate='intersects')

        # Routine to generate the confusion matrix.
        CONFUSION_BINS = [0, 0.01, 0.25, 0.50, 0.75, 1]
        CONFUSION_LABELS = ['0.00 to 0.01', '0.01 to 0.25',
        ...
        intersection = intersection.dropna(
            subset=['indicator_label', 'estimation_label'])
        intersection['indicator_label'] = intersection['indicator_label'].values.astype(
            'string')
        intersection['estimation_label'] = intersection['estimation_label'].values.astype(
            'string')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--indicator_files_mask", required=True,
                        help="Path to the directory where the indicator file masks are located. The glob will be used to find the indicator files.")
