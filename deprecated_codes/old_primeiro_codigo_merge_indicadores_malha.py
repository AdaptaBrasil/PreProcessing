...
def carregaMalha(caminho_malha):
    ...
    if malha.crs.to_epsg() != 5880:
        ...
        # <- Verificar se o padrão está correto e se vai precisar de ajustes
        malha = malha.to_crs(CRS.from_epsg(5880))
...
def main(args):
    ...
    if DEBUG:
        ...
        print("Novo arquivo de malha atualizado:",
              caminho_nova_malha_atualizada)
        print("Calcular pelo valor máximo, não pela média:", args.media)
    ...
    if nextColId > 1:
        print("Arquivo de relação de colunas encontrado! Abri-lo para continuar o processamento.")
        df_dados_relacao = pd.read_excel(nome_arquivo_relacao_colunas)
    else:
        print("Arquivo de relação de colunas não encontrado! Criando um novo arquivo.")
        dados_relacao = {
            'nome_arquivo': [],
            'coluna': []
        }
    ...
    for i, caminho_arquivo_indicador in enumerate(arquivos_shp):
        try:
            ...
            ja_processado = df_dados_relacao.loc[df_dados_relacao['nome_arquivo']
                                                 == nome_arquivo_solo]
            ...
            if not(colName.startswith('ERRO":')):
                ...
                print(colName)
                nova_linha = {
                    'nome_arquivo': nome_arquivo_solo, 'coluna': colName}
                df_dados_relacao = df_dados_relacao._append(
                    nova_linha, ignore_index=True)
                continue
            ...
            intersecao = gpd.sjoin(
                malha, indicador, how='inner', op='intersects')

            # Criar GeoDataFrame com os campos desejados
            gdf = gpd.GeoDataFrame(
                intersecao[['ID', 'objectid']], geometry=intersecao.geometry)
            ...
            # Por exemplo, atribuir o valor 10 a todos os registros
            malha[chave_coluna_i] = -1.0

            # Exibir o GeoDataFrame com a nova coluna
            if DEBUG:
                print(f"Coluna {chave_coluna_i} em malha: ",
                      malha[chave_coluna_i])
            ...
            for indice_malha, linha_i_malha in malha.iterrows():
                ...
                linhas_intersecoes = intersecao.loc[intersecao['objectid']
                                                    == objectid_i_malha]
                ...
                for _, linha_j_intersecao in linhas_intersecoes.iterrows():
                    ...
                    linha_volume = indicador.loc[indicador['ID']
                                                 == ID_j_intersecao]

                    # Obter a área da interseção
                    area_interseccao = geometry_i_fmalha.intersection(
                        geometry_j_intersecacao).area
                ...
                malha.at[indice_malha,
                         chave_coluna_i] = media_ponderada if args.media else max_valor
                # if DEBUG:
                #   print(f"Valores para {indice_malha + nextColId}: Média: {media_ponderada} Máxima: {max_valor}")

            # Nova linha a ser adicionada
            nova_linha = {'nome_arquivo': nome_arquivo_solo,
                          'coluna': chave_coluna_i}
            ...
            df_dados_relacao.to_excel(
                nome_arquivo_relacao_colunas, index=False)

            # Salvar malha
            malha.to_file(caminho_nova_malha_atualizada)

            print("\nNova malha atualizada salva em: ",
                  caminho_nova_malha_atualizada)
            print("Arquivo relacao colunas salvo em: ",
                  nome_arquivo_relacao_colunas)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mascara_indicadores", required=True,
                        help="Caminho da máscara de arquivos indicadores. Será usado o glob para encontrar os arquivos indicadores")
    parser.add_argument("--arquivo_malha", required=True,
                        help="Caminho do arquivo de malha")
    parser.add_argument("--arquivo_relacao", required=True,
                        help="Caminho do arquivo de relação de colunas")
    parser.add_argument("--nova_malha", required=True,
                        help="Caminho do novo arquivo de malha atualizado")
    ...
    tempo_em_minutos = (tempo_final - tempo_inicio) / 60
    print(f"Tempo de execução: {tempo_em_minutos} minutos")